...
public class TargetWithTalk {
    ...
    public static OneShot<LivingEntity> create(Predicate<LivingEntity> predicate, float f) {
        float g = f * f;
        return BehaviorBuilder.create((instance) -> instance.group(instance.absent(MemoryRegistry.TALK_TARGET.get()), instance.present(MemoryModuleType.NEAREST_VISIBLE_LIVING_ENTITIES), instance.absent(MemoryRegistry.TALK_COOLDOWN.get())).apply(instance, (memoryAccessor, memoryAccessor2, memoryAccessor3) -> (serverLevel, livingEntity, l) -> {
