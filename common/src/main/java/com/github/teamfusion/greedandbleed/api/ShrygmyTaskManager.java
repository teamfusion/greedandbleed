...
public class ShrygmyTaskManager<T extends Shrygmy> extends TaskManager<T> {
    ...
    protected void initWorkActivity(int priorityStart) {
        dynamicBrain.addActivityAndRemoveMemoryWhenStopped(Activity.WORK, priorityStart, ImmutableList.of(StartAttacking.create(ShrygmyTaskManager::findNearestValidAttackTarget), createIdleLookBehaviors(), createWorkMovementBehaviors()), MemoryRegistry.WORK_TIME.get());
