...
public class GBOnAStickWithHoglinItem extends FoodOnAStickItem<Pig> {
    private final int consumeItemDamage;

    public GBOnAStickWithHoglinItem(Properties properties, int consumeItemDamage) {
    ...
    public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand hand) {
        ...
        if (!level.isClientSide) {
            ...
            boolean canInteractWith = player.isPassenger() && vehicle != null && vehicle instanceof Hoglin;
            if (canInteractWith) {
